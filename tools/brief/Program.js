...
class Program {
    ...
    static main(root, extension, script) {
        ...
        Logger.debug("Retrieving ImGui extensions files...", {type: "ImExts"});
        const exts = this.readHeaderDefineDirectives(new FileEditor(root_imext + "config.h", true), true);
        Logger.debug(`Found ${Object.keys(exts).length} extensions in "${root_imext}config.h"`, {type: "ImExts"});

        Logger.info("Retrieving ImGui extensions...", {type: "ImExts"});
        Object.entries(exts).forEach(([d, extval]) => {
            let extFolder, extName, extNameS, extNameO, extNameP, extHeader;
            if (typeof extval === "string") {
                extFolder = extval;
                extNameO = d.replace(/^IMEXT_/, "").toLowerCase();
                extHeader = undefined;
            } else {
                extFolder = extval[0];
                if (extval.length > 1) {
            ...
            if (extNameO.toLowerCase().startsWith("imext")) {
                extNameO = extNameO.slice(5);
            }

            extName = Util.toCamelCase(extNameO);
            extNameS = Util.toSnakecase(extName);
            extNameP = `${extName.slice(0,1).toUpperCase() + extName.slice(1)}`;

            for (const hAttempt of ["imext.h", "imext_" + extNameS + ".h", extNameS + ".h"]) {
                if (fs.existsSync(path.normalize(root_imext + extFolder + path.sep + hAttempt))) {
                    extHeader = hAttempt;
                    break;
                }
            }
            const extHeaders = fs.readdirSync(root_imext + extFolder + path.sep + "extension").filter(e => e.endsWith(".h"));
            if (extHeaders.length === 0 && extHeader === undefined) throw `Could not run program, could not find any wrapper files in "${root_imext + extFolder}"`;

            exts[d] = {NameO: extNameO, Name: extName, NameS: extNameS, NameP: extNameP, Folder: extFolder, Headers: [extHeader, ...extHeaders.map(e => `extension/${e}`)]};
            Logger.info(`Found ImGui extension at ${exts[d].Folder}`, {type: `ImExts/${exts[d].NameP}`});
        });
        ...
        Object.entries(exts).forEach(([extDirective, extData]) => {
            ...
            Logger.debug(`Parsing ImGui extension wrappers of ${extN}...`, {type: "ImExt"});
            const files = fs.readdirSync(root_imext + extData.Folder).filter(e => e.endsWith("_gm.cpp"));
            if (files.length === 0) throw `Could not run program, could not find any wrapper files in "${root_imext + extData.Folder}"`;
            Logger.debug(`Found ${files.length} wrapper files in "${root_imext + extData.Folder}"`, {type: `ImExt/${extNameP}`});
            files.forEach(e => {
                this.parseWrapper(wrappers, new FileEditor(`${root_imext + extData.Folder}/${e}`, true), extN);
            })
        });
        ...
        Object.entries(exts).forEach(([extDirective, extData]) => {
            ...
            if (!fs.existsSync(path.normalize(extScript))) {
                throw `ImGui extension GML script "${extScript}" not found. please create it in your GameMaker project.`;
            }
            extData._HeadersParsed = {enums: {}, functions: []};

            let extHeader;

            for (const h of extData.Headers.reverse()) {
                extHeader = {enums: {}, functions: []};
                if (h !== undefined) {
                    extHeader = this.parseHeader(new FileEditor(root_imext + extData.Folder + path.sep + h), extNameP);
                    extData._HeadersParsed.functions.push(...extHeader.functions);
                    for(const key in extHeader.enums) {
                        extData._HeadersParsed.enums[key] = extHeader.enums[key];
                    }
                }
            }

            const extWrappers = wrappers.filter(w => (w.Context === extN));
            extData._Wrappers = extWrappers;

            this.writeScripts(extWrappers, extData._HeadersParsed.enums, new FileEditor(extScript), extNameP);
        });
        ...
        if (Configuration.WRITE_REPORT) {
            ...
            Object.entries(exts).forEach(([extDirective, extData]) => {
                ...
                extData._CoveragePercentage = this.writeReport(extHeader, extWrappers, new FileEditor(p), extNameP);
            ...
            if (Configuration.WRITE_BADGES) {
                ...
                Object.entries(exts).forEach(([extDirective, extData]) => {
                    const p = `extra/badges/coverage_${extData.NameS}.json`;
                    const fp = path.normalize(p);
                    if (!fs.existsSync(fp)) {
                        fs.writeFileSync(fp, "");
                    }
                    this.writeCoverageBadge(new FileEditor(p), extData._CoveragePercentage, extData.NameP, "darkgreen");
                });
            }
        }
